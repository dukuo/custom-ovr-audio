...
function clipLinePlane(plane, p0, p1, uv0, uv1) {
  let d0 = p0.x * plane[0] + p0.y * plane[1] + p0.z * plane[2] + p0.w * plane[3];
  let d1 = p1.x * plane[0] + p1.y * plane[1] + p1.z * plane[2] + p1.w * plane[3];
  if (d0 < 0 && d1 < 0) {
    return true;
  }
  if (sign(d0) != sign(d1)) {
...
HPanoBufferGeometry.prototype = Object.assign( Object.create( THREE.BufferGeometry.prototype ), {
  ...
  dispose: function() {
    ...
    for (let i in this.material.materials) {
  ...
  update: function(maxLevels, projectionMatrix) {
    ...
    // bl - bottomLeft
    // br - bottomRight
    function fillQuad(ctx,
      tl, tr, bl, br,
      tile,
      side,
      level) {
      // replace markup for texture url
      let file = geom.baseurl.
      ...
      if (projectionMatrix) {
        ...
        let ctx = {texelPerPixel:1}
        let anyVisible = false;
        anyVisible |= !clipLineToFrustum(ctx, tl, br, [0,0], [tileSize, tileSize], projectionMatrix);
        anyVisible |= !clipLineToFrustum(ctx, tl, tr, [0,0], [0, tileSize], projectionMatrix);
        anyVisible |= !clipLineToFrustum(ctx, tr, br, [tileSize,0], [tileSize, tileSize], projectionMatrix);
        anyVisible |= !clipLineToFrustum(ctx, br, bl, [tileSize, tileSize], [0,0], projectionMatrix);
      ...
      if (mtr.loadState != HPANO_MAP_LOADED || level === maxLevels || !divide) {
        ...
        let valid = true;
        valid &= fillQuad(ctx,
          tl, mid_tltr, mid_tlbl, mid_tlbr,
          [tile[0]*2,tile[1]*2],
          side,
          level+1);
        valid &= fillQuad(ctx,
          mid_tltr, tr, mid_tlbr, mid_trbr,
          [tile[0]*2+1,tile[1]*2],
          side,
          level+1);
        valid &= fillQuad(ctx,
          mid_tlbl, mid_tlbr, bl, mid_blbr,
          [tile[0]*2,tile[1]*2+1],
          side,
          level+1);
        valid &= fillQuad(ctx,
          mid_tlbr, mid_trbr, mid_blbr, br,
    ...
    for (let i in this.materialsCached) {
