...
import RCTMesh from '../Views/Mesh';
...
export default class UIManager extends Module {
  ...
  customBubblingEventTypes: {[event: string]: {
  ...
  constructor(rnctx: ReactNativeContext, guiSys: GuiSys) {
    ...
    this.customDirectEventTypes = {
      ...
      topGazeEnter: {registrationName: "onGazeEnter"},
      topGazeExit: {registrationName: "onGazeExit"},
      topMouseEnter: {registrationName: "onMouseEnter"},
      topMouseExit: {registrationName: "onMouseExit"},
    };
    this.customBubblingEventTypes = {
      topChange: {phasedRegistrationNames : {bubbled: "onChange", captured: "onChangeCapture"}},
      topInput: {phasedRegistrationNames : {bubbled: "onInput", captured: "onInputCapture"}},
      topHeadPose: {phasedRegistrationNames : {bubbled: "onHeadPose", captured: "onHeadPoseCapture"}},
      topGazeInput: {phasedRegistrationNames : {bubbled: "onGazeInput", captured: "onGazeInputCapture"}},
      topGazeHeadPose: {phasedRegistrationNames : {bubbled: "onGazeHeadPose", captured: "onGazeHeadPoseCapture"}},
      topMouseInput: {phasedRegistrationNames : {bubbled: "onMouseInput", captured: "onMouseInputCapture"}},
      topMouseHeadPose: {phasedRegistrationNames : {bubbled: "onMouseHeadPose", captured: "onMouseHeadPoseCapture"}},
    };
  ...
  manageChildren(
    ...
    const numToAdd = !addChildTags ? 0 : addChildTags.length;
    const numToRemove = !removeFrom ? 0 : removeFrom.length;
   ...
   */
  registerViewType(name: string, view: RCTBaseView, viewCreator: () => RCTBaseView) {
    this[name] = view;
  ...
  purgeView(view: RCTBaseView) {
    ...
    const type = this._viewTypes[view.tag];
    delete this._viewTypes[view.tag];
    delete this._views[view.tag];
    delete this._viewsOfType[type][view.tag];
    // purge children
  ...
  frame() {
    ...
    for (let tag in this._views) {
      this._views[tag].frame();
    }
    // layout the views
    this.layout();
  }
  ...
  layout() {
    ...
    for (let tag in this._rootViews) {
      ...
      // invalidate the width and height
      rootView.layout.width = undefined;
      rootView.layout.heigth = undefined;
      // use css-layout flex box to apply new flow
      computeLayout(rootView);
  ...
  setBoundingBoxVisible(tag: number, visible: boolean) {
    ...
    if (visible) {
      ...
      const boundViz = new THREE.Mesh(
        new THREE.BoxGeometry(bounds.max.x - bounds.min.x, bounds.max.y - bounds.min.y, bounds.max.z - bounds.min.z, 1, 1, 1),
  ...
  measure(reactTag: number, callback: number) {
    ...
    let x = 0;
    let y = 0;
    let w = view.layout.width;
    let h = view.layout.height;
    while (view) {
      x += (view.layout.left - view.layout.width * view.style.layoutOrigin[0]);
      y += (view.layout.top - view.layout.height * view.style.layoutOrigin[1]);
      view = view.getParent();
    }
  ...
  measureLayoutRelativeToParent(
    ...
    let x = (view.layout.left - view.layout.width * view.style.layoutOrigin[0]);
    let y = (view.layout.top - view.layout.height * view.style.layoutOrigin[1]);
    let w = view.layout.width;
    let h = view.layout.height;
    this._rnctx.invokeCallback(successCallback, [x, y, w, h]);
  }
