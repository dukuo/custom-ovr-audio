...
describe('OBJParser Utilities', () => {
  it('can read a series of 3 numeric points', () => {
    ...
    expect(() => OBJParser.readPoints(' 145 ', 0)).toThrow('Expected a series of numbers on line 0');
    expect(() => OBJParser.readPoints(' abc def ghi', 0)).toThrow('Expected a series of numbers on line 0');
  });
  ...
  it('can read a series of triplets', () => {
    expect(OBJParser.readTriplets(' 1/2/3 45/67/89 10/11/12')).toEqual([[1,2,3], [45,67,89], [10,11,12]]);
...
# DONE`
      ...
      expect(obj.geometry.position).toEqual([
        0,0,0,
        10,10,0,
        0,10,0,
        10,0,0,
      ]);
      expect(obj.geometry.index).toEqual([
        0, 1, 2, 0, 3, 1
      ]);
      expect(obj.geometry.uv).toEqual([
        0,0,
        10,10,
        0,10,
        10,0,
...
`
      ...
      expect(state.objects[4].geometry.position).toEqual([
        0,-2,0,
        0,0,-2,
        2,0,0,
      ]);
      ...
      expect(state.objects[6].geometry.position).toEqual([
        -2,0,0,
        0,0,2,
        0,2,0,
      ]);
