...
type NestedArr = Array<number> | Array<Array<number>> | Array<Array<Array<number>>>;
 ...
 */
export function readTexturePoints(remainder: string, lineNumber: number): Array<number> {
 ...
 */
export function readTriplets(remainder: string, lineNumber: number): Array<[number, number, number]> {
  const triplets = [];
  while (remainder.length > 0) {
...
export function readLine(state: OBJParserState, line: string, lineNumber: number) {
  ...
  if (first === 'g' || first === 'o') {
    if (line[index + 1] === ' ' || line[index + 1] === '\t') {
      ...
      if (state.currentObject && state.currentObject.materials.length > 0) {
        ...
        const inherited = state.currentObject.materials[
  ...
  if (first === 'u') {
    if (line.substr(index, 6) === 'usemtl') {
      ...
      const lib =
        state.materialLibraries[state.materialLibraries.length - 1];
...
export function readOBJFile(data: string): Promise<OBJParserState> {
  ...
  const textureCoords = [];
  const normals = [];
  const starterObject = new OBJGroup(
    vertices,
    textureCoords,
    normals
  ); // In case no object group is declared, we start with something
  const state = {
    vertices: vertices,
